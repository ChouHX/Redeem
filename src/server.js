...
import { fail, formatDateTime, normalizeEmail, ok } from "./utils.js";
...
const DEFAULT_AD_SLOT = {
  enabled: true,
  title: "自营发卡推荐",
  description: "本站同步提供多类数字商品，可直接前往发卡网下单。",
...
function ensureRedeemDataAvailable(codeInfo, res) {
  ...
  if (codeInfo.data_deleted_at || isRedeemAccessExpired(codeInfo.redeemed_at)) {
    const expired = isRedeemAccessExpired(codeInfo.redeemed_at);
    res
      .status(410)
      .json(
        fail(
          expired || codeInfo.data_deleted_reason === "expired_24h"
            ? "兑换已超过24小时，在线访问已关闭；取件请手动导入账号数据"
            : "该卡密对应的账号信息已删除；取件请手动导入账号数据",
        ),
      );
    return false;
  }
...
function normalizeAdSlotValue(value, fallback) {
  return {
    ...
    description: String(
      value?.description || fallback.description || "",
    ).trim(),
    ticker_text: String(
      value?.ticker_text || fallback.ticker_text || "",
    ).trim(),
    ...
    items: Array.isArray(value?.items)
      ...
      : Array.isArray(value?.highlights)
        ? value.highlights
            ...
            .slice(0, 6)
        : fallback.items,
    primary_action: normalizeAdAction(
      value?.primary_action,
      fallback.primary_action,
    ),
  };
}
...
function formatDbErrorMessage(error, fallback) {
  ...
  if (
    message.includes("redeem_codes.code") ||
    message.includes("redeem_codes.normalized_code")
  ) {
    return "兑换码生成冲突，请重试";
  }
...
function parseMailFolder(value) {
  const normalized = String(value || "inbox")
    .trim()
    .toLowerCase();
...
async function fetchStoredAccountMessagesPaged(email, options = {}) {
  ...
  const result = await getAccountMailMessagesPaged(
    { email, ...account },
    options,
  );
  await persistRefreshedAccountToken(email, account, result.refreshToken);
...
async function fetchStoredAccountMessageDetail(email, options = {}) {
  ...
  const result = await getAccountMailMessageDetail(
    { email, ...account },
    options,
  );
  await persistRefreshedAccountToken(email, account, result.refreshToken);
...
app.post("/api/temp-messages", async (req, res) => {
  try {
    ...
    const result = await getAccountMessagesWithContent(payload, top, folder);
...
app.post("/api/redeem/access", async (req, res) => {
  ...
  const page = parseBoundedInt(req.body?.page, 1, { min: 1, max: 100000 });
  const pageSize = parseBoundedInt(req.body?.page_size, 10, {
    min: 1,
    max: 100,
  });
  if (!code) {
  ...
  try {
    ...
    if (!ensureRedeemDataAvailable(codeInfo, res)) {
      return;
    }
...
app.post("/api/redeem/exchange", (req, res) => {
  ...
  try {
    const codeInfo = getRedeemCodeByCode(code);
    if (codeInfo && !ensureRedeemDataAvailable(codeInfo, res)) {
      return;
    }
...
async function callGeminiProUpstream(
  pathname,
...
app.get("/api/geminipro/tasks/:cardCode", async (req, res) => {
  ...
  const pageSize = parseBoundedInt(req.query.page_size, 20, {
    min: 1,
    max: 100,
  });
...
app.post("/api/geminipro/submit", async (req, res) => {
  ...
  const accountsInput = Array.isArray(req.body?.accounts)
    ? req.body.accounts
    : [];
  ...
  if (invalidAccount) {
    res.status(400).json(fail("账号格式应为：账号--密码--2FA密钥"));
    return;
  }
  ...
  try {
    ...
    if (response.payload?.success === false) {
      res.status(400).json(fail(response.payload.message || "任务提交失败"));
      return;
    }

    const data = response.payload?.data ?? response.payload ?? null;
    res.json(
      ok(data, response.payload?.message || `已提交 ${accounts.length} 条账号`),
    );
  } catch (error) {
    if (error?.name === "AbortError") {
...
app.post(
  ...
  (req, res) => {
    ...
    try {
      const inventory = updateRedeemInventoryStatus(
        inventoryId,
        req.body?.status,
      );
      if (!inventory) {
...
app.post(
  ...
  (req, res) => {
    ...
    const typeId = parseOptionalBoundedInt(req.body?.type_id, {
      min: 1,
      max: 1000000000,
    });
    ...
    const nextStatus = hasBodyValue(req.body?.status)
...
app.post(
  ...
  (req, res) => {
    ...
    try {
      const result = rollbackRedeemCode(codeId);
      if (!result) {
        res.status(404).json(fail("兑换码不存在"));
        return;
      }
      res.json(
        ok(
          result,
          `卡密已销毁，${result.restored_inventory_count} 个账号已重新入库为可用状态`,
        ),
      );
    } catch (error) {
      res.status(400).json(fail(error.message || "卡密回退失败"));
    }
  },
);
...
app.post("/api/redeem/admin/codes/export", requireAdmin, (req, res) => {
  ...
  const filenameTag = codeIds.length
    ? "selected"
    : sanitizeExportCell(filters.status || "filtered").toLowerCase() ||
      "filtered";
